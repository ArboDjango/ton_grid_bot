...
Tests couvrant les étapes 5 et 6 du plan de reconstruction : la
migration du PnL réalisé (bot_gateio.py) vers le
CapitalTransitionGuard, TransitionTypes REALIZED_PROFIT (étape 5) et
REALIZED_LOSS (étape 6).

Portée strictement respectée :
  - Aucun test ne réévalue le calcul du profit ou de la perte
    eux-mêmes (FIFO, frais, prix moyen, Gv) : ces calculs restent
    entièrement dans bot_gateio.py et ne sont pas exercés ici. Les
    montants utilisés dans ces tests sont fournis directement, comme
    le ferait bot_gateio.py après avoir calculé pnl_trade.
  ...
  - Ces tests vérifient, pour chacune des deux causes :
      * build_realized_profit_request / build_realized_loss_request
        produisent une TransitionRequest correcte, avec le montant
        transmis strictement inchangé ;
      * l'intégration bout-en-bout (Guard + StateDictEconomicRepository,
        déjà validé à l'étape 3) applique correctement le montant à
        allocated_capital ;
      * une entrée de journal est créée avec le bon montant et la
        bonne cause ;
...
# ============================================================
# BUILD_REALIZED_LOSS_REQUEST
# ============================================================

class TestBuildRealizedLossRequest:
    def test_produces_a_realized_loss_transition_request(self):
        request = build_realized_loss_request(
            bot_id="gateio_rsrusdt", amount=-8.5, justification="test"
        )

        assert request.bot_id == "gateio_rsrusdt"
        assert request.cause is TransitionCause.REALIZED_LOSS
        assert request.origin is TransitionOrigin.BOT

    def test_amount_is_transmitted_exactly_unchanged(self):
        # Le montant transmis au Guard doit etre exactement celui deja
        # calcule par bot_gateio.py (pnl_trade, negatif), sans arrondi
        # ni transformation d'aucune sorte.
        pnl_trade = -4.987654321
        request = build_realized_loss_request(bot_id="bot_1", amount=pnl_trade)

        assert isinstance(request.value, AbsoluteAmount)
        assert request.value.amount == pnl_trade

    def test_justification_defaults_to_empty_string(self):
        request = build_realized_loss_request(bot_id="bot_1", amount=-5.0)
        assert request.justification == ""
    ...
    def test_does_not_validate_or_reject_a_non_negative_amount(self):
        # Symetrique a build_realized_profit_request : cette fonction
        # ne decide jamais si un montant est une perte. C'est a
        # l'appelant (bot_gateio.py) de ne l'invoquer que lorsque
        # pnl_trade < 0.
        request = build_realized_loss_request(bot_id="bot_1", amount=3.0)
        assert request.value.amount == 3.0

    def test_is_pure_same_inputs_yield_equal_requests_value(self):
        request_a = build_realized_loss_request(bot_id="bot_1", amount=-10.0)
        request_b = build_realized_loss_request(bot_id="bot_1", amount=-10.0)
        assert request_a.value == request_b.value
        assert request_a.cause == request_b.cause
        assert request_a.origin == request_b.origin
...
class TestRealizedLossEndToEnd:
    def _make_guard(self, state, bot_id):
        repository = StateDictEconomicRepository(state, bot_id, save_fn=lambda s: None)
        journal = CapitalTransitionJournal()
        guard = CapitalTransitionGuard(repository=repository, journal=journal)
    ...
    def test_loss_decreases_allocated_capital_by_exactly_the_amount(self):
        state = {"allocated_capital": 220.0}
        guard, journal = self._make_guard(state, "gateio_rsrusdt")

        pnl_trade = -6.789  # tel que calcule par bot_gateio.py (FIFO, net de frais)
        request = build_realized_loss_request(bot_id="gateio_rsrusdt", amount=pnl_trade)
        result = guard.submit_transition(request)
    ...
    def test_creates_exactly_one_journal_entry_with_the_correct_cause(self):
        ...
        request = build_realized_loss_request(bot_id="bot_1", amount=-5.5)
        guard.submit_transition(request)

        history = journal.history_for("bot_1")
        assert len(history) == 1
        entry = history[0]
        assert entry.cause is TransitionCause.REALIZED_LOSS
        assert entry.origin is TransitionOrigin.BOT
        assert entry.status is TransitionStatus.ACCEPTED
        assert entry.state_before == EconomicState(allocated_capital=100.0)
        assert entry.state_after == EconomicState(allocated_capital=94.5)

    def test_is_retrievable_via_guard_get_history(self):
        state = {"allocated_capital": 100.0}
        guard, journal = self._make_guard(state, "bot_1")

        request = build_realized_loss_request(bot_id="bot_1", amount=-20.0)
        guard.submit_transition(request)

        history = guard.get_history("bot_1")
        assert len(history) == 1
        assert history[0].cause is TransitionCause.REALIZED_LOSS

    def test_repository_state_dict_reflects_new_allocated_capital(self):
        state = {"allocated_capital": 50.0}
        guard, journal = self._make_guard(state, "bot_1")

        request = build_realized_loss_request(bot_id="bot_1", amount=-12.5)
        guard.submit_transition(request)

        assert state["allocated_capital"] == pytest.approx(37.5)

    def test_other_state_fields_remain_untouched(self):
        # Absence de regression : seul allocated_capital est modifie ;
        # tout ce qui concerne FIFO/inventaire/PnL cumule/grille reste
        # strictement inchange par ce mecanisme.
        state = {
            "allocated_capital": 100.0,
        ...
        request = build_realized_loss_request(bot_id="bot_1", amount=-7.0)
        guard.submit_transition(request)
        ...
        assert state["buy_grid"] == [0.9, 0.8]
        assert state["wallet_peak"] == 999.0
        assert state["allocated_capital"] == pytest.approx(93.0)

    def test_multiple_successive_losses_accumulate_correctly(self):
        # Simule plusieurs ventes deficitaires successives : chaque
        # transition s'applique sur l'etat resultant de la precedente,
        # jamais sur un etat fige.
        state = {"allocated_capital": 200.0}
        guard, journal = self._make_guard(state, "bot_1")
    ...
    def test_does_not_affect_a_different_bot(self):
        ...
        guard_1.submit_transition(
            build_realized_loss_request(bot_id="bot_1", amount=-10.0)
        )

        assert state_bot_1["allocated_capital"] == pytest.approx(90.0)
        assert state_bot_2["allocated_capital"] == pytest.approx(300.0)

    def test_profit_and_loss_can_alternate_on_the_same_bot(self):
        # Scenario realiste : une vente profitable suivie d'une vente
        # deficitaire sur le meme bot, chacune appliquee correctement
        # sur l'etat resultant de la precedente.
        state = {"allocated_capital": 200.0}
        guard, journal = self._make_guard(state, "bot_1")

        guard.submit_transition(
            build_realized_profit_request(bot_id="bot_1", amount=15.0)
        )
        guard.submit_transition(
            build_realized_loss_request(bot_id="bot_1", amount=-6.0)
        )

        assert state["allocated_capital"] == pytest.approx(200.0 + 15.0 - 6.0)
        history = journal.history_for("bot_1")
        assert len(history) == 2
        assert history[0].cause is TransitionCause.REALIZED_PROFIT
        assert history[1].cause is TransitionCause.REALIZED_LOSS
